...
export const addUser = async (id,evt) => {
  await clickhouse.query('INSERT INTO users (btoken, date, id,username,realname) VALUES ('+btoken+',now(),'+id+',\''+getUsernameFromEvent(evt)+'\',\''+getRealnameFromEvent(evt)+'\');').toPromise();
  db.get('users').push({ id, rank: 0, version, left: false }).value();
}
